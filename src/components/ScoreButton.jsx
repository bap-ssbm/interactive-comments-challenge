...
function ScoreButton(props) {
    ...
    const [numOfVotes, updateNumVotes] = useState(parseInt(props.score));

    const upVoteMatch = props.user.voted.some(vote => {
        return vote.votedID === props.id && vote.typeOfVote === "upvote";
    });
    const downVoteMatch = props.user.voted.some(vote => {
        return vote.votedID === props.id && vote.typeOfVote === "downvote";
    });
    ...
    function upvote(event){
        ...
        const match = props.user.voted.some(vote => {
            return vote.votedID === props.id && vote.typeOfVote === "upvote";
        });

        const oppositematch = props.user.voted.some(vote => {
            return vote.votedID === props.id && vote.typeOfVote === oppositeType;
        });

        if(match){
            let newVote = numOfVotes+1;
            props.updateVote(props.id, newVote, "upvote");
    ...
    function downvote(event){
        ...
        if(match){
            let newVote = numOfVotes-1;
            props.updateVote(props.id, newVote, "downvote");
        }else if(oppositematch){
            let newVote = numOfVotes-1;
            updateVotedDown(false);
            updateVotedUp(false);
    ...
    return (<div className="vote-section"> 
        <form className="vote-wrapper">
            <button className="upvote" name={numOfVotes} value = {props.id} onClick={upvote}>
            ...
            <button  value = {props.id} name={numOfVotes} className="downvote" onClick={
                downvote}>
